...
features = ['temp', 'humidity', 'wind_speed', 'wind_dir', 'rainfall', 'pressure','aod550']
target = 'pm25'
SEQ_LEN = 168  # 7 days
HORIZON = 24   # next 24h
...
for city in cities:
    ...
    plt.title(f"GNN MAMBA Forecast with AOD - {city}")
    plt.xlabel("Datetime")
    plt.ylabel("PM2.5")
...
plt.ylabel("NMSE")
plt.title("NMSE per Forecast Horizon (1–24h) across all Cities (GNN + MAMBA with AOD)")
plt.legend()
